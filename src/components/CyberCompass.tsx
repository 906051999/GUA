...
export function CyberCompass({ className, intensity = 1 }: CyberCompassProps) {
  ...
  useEffect(() => {
    ...
    const draw = (t: number) => {
      ...
      g.addColorStop(0, `rgba(0, 243, 255, ${0.22 * pulse})`);
      g.addColorStop(0.55, `rgba(0, 243, 255, ${0.05 * pulse})`);
      ...
      ctx.lineWidth = 1.2;
      ctx.strokeStyle = `rgba(0, 243, 255, ${0.68 * pulse})`;
      ctx.shadowBlur = 18;
      ...
      ctx.shadowBlur = 0;
      ctx.lineWidth = 1;
      ctx.strokeStyle = `rgba(255, 215, 0, ${0.55 + 0.25 * pulse})`;
      ctx.beginPath();
...
function drawBagua(ctx: CanvasRenderingContext2D, radius: number, pulse: number) {
  ...
  ctx.fillStyle = `rgba(0, 243, 255, ${0.75 * pulse})`;
  ...
  ctx.strokeStyle = `rgba(255, 51, 51, ${0.55 + 0.25 * pulse})`;
  ctx.lineWidth = 1.1;
  for (let i = 0; i < 8; i += 1) {
...
function drawCodeRing(ctx: CanvasRenderingContext2D, radius: number, pulse: number) {
  ...
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.fillStyle = `rgba(0, 243, 255, ${0.28 + 0.12 * pulse})`;
